...
Rule_Constraint_Reference_Type::
Rule_Constraint_Reference_Type( xmlNodePtr root, World* world ) : Rule() {
...
evaluate_cv( const Grounding_Set* groundingSet )const{
  ...
  for( unsigned int i = 0; i < groundingSet->groundings().size(); i++ ){
    if( dynamic_cast< const Rule_Constraint_Reference_Type* >( groundingSet->groundings()[ i ] ) ){
      if( *this == *static_cast< const Rule_Constraint_Reference_Type* >( groundingSet->groundings()[ i ] ) ){
        cv = "true";
      }
    }
  }
...
fill_rules( const World* world, Grounding_Set* groundingSet )const{
  Rule_Constraint_Reference_Type rule_constraint_reference_type( constraint_reference_type() );
...
equals( const Grounding& other )const{
  if( dynamic_cast< const Rule_Constraint_Reference_Type* >( &other ) != NULL ){
    return ( *this == *static_cast< const Rule_Constraint_Reference_Type* >( &other ) );
  } else {
    return false;
  }
}
...
to_latex( void )const{
  stringstream tmp;
  tmp << "Rule_Constraint_Reference_Type(constraint_reference_type=" << constraint_reference_type() << ")";
  return tmp.str();
}
