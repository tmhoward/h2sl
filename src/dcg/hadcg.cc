 ...
 *   Adaptive Distributed Correspondence Graph
 */
...
HADCG::
HADCG( const HADCG& other ) : _dcg( other._dcg ),
...
leaf_search( const Phrase* phrase,
  ...
  if( phrase != NULL ){
    ...
    for( vector< pair< double, Symbol_Dictionary > >::const_iterator it_symbol_dictionary = _inferred_symbol_dictionaries.begin(); it_symbol_dictionary != _inferred_symbol_dictionaries.end(); it_symbol_dictionary++ ){
      ...
      _adcg.leaf_search( phrase, it_symbol_dictionary->second, searchSpace, world, context, llm, beamWidth, debug );
  
      for( vector< pair< double, Phrase* > >::const_iterator it_solution = _adcg.solutions().begin(); it_solution != _adcg.solutions().end(); it_solution++ ){
