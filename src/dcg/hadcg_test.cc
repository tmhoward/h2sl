...
main( int argc,
  ...
  for( unsigned int i = 0; i < args.inputs_num; i++ ){
    ...
    if( !args.grammar_given ){    
      ...
      clear( phrases.back() );
      hadcg->leaf_search( phrases.back(), *symbol_dictionary, search_space, world, context, llm, args.beam_width_arg, ( bool )( args.debug_arg ) );
      if( !hadcg->solutions().empty() ){
        cout << "num_phrases: " << to_std_string( hadcg->solutions().front().second->num_phrases() ) << endl;
        cout << "search space( abstract_max_size ) : " << to_std_string( hadcg->solutions().front().second->aggregate_property_phrases( std::string( "abstract_max_size" ) ) ) << endl;
        cout << "search space( abstract_avg_size ) : " << to_std_string( hadcg->solutions().front().second->aggregate_property_phrases( std::string( "abstract_avg_size" ) ) ) << endl;
        cout << "search space( avg_abstract_avg_size ) : " << to_std_string( hadcg->solutions().front().second->statistic_aggregate_property_phrases( "abstract_avg_size", "per-phrase-avg" ) ) << endl;
        cout << "search space( avg_abstract_max_size ) : " << to_std_string( hadcg->solutions().front().second->statistic_aggregate_property_phrases( "abstract_max_size", "per-phrase-avg" ) ) << endl;
        cout << "search space( avg_concrete_size ) : " << to_std_string( hadcg->solutions().front().second->statistic_aggregate_property_phrases( "concrete_size", "per-phrase-avg" ) ) << endl;
        cout << "search space( concrete_size ) : " << to_std_string( hadcg->solutions().front().second->aggregate_property_phrases( std::string( "concrete_size" ) ) ) << endl;

        cout << "solution: " << *hadcg->solutions().front().second << " (" << hadcg->solutions().front().first << ")" << endl;
    ...
    if( args.output_given ){
      output_filename << args.output_arg << "/hadcg_solution_" << example_number_string << ".xml";
      hadcg->solutions().front().second->to_xml( output_filename.str() );
    } else{
      output_filename << "/tmp/hadcg_solution_" << example_number_string << ".xml";
      hadcg->solutions().front().second->to_xml( output_filename.str() );
    }
