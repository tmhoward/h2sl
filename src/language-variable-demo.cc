...
int main( int argc, char* argv[] ){
  ...
  auto child_language_variable = std::make_shared<h2sl::LanguageVariable>("NP", "np", h2sl::LanguageVariable::symbolsVector(),
  ...
  i_language_variable.from_xml( filename, symbolspace.get() );
  ...
  if( vm.count("input") ){
    std::cout << "Importing a LanguageVariable from \"" << vm["input"].as<std::string>()
              << "\"" << std::endl;
    default_language_variable.from_xml( vm["input"].as<std::string>().c_str(), symbolspace.get() );
