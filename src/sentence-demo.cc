...
int main( int argc, char* argv[] ){
  ...
  auto np = std::make_shared<h2sl::LanguageVariable>("NP");
  np->words = std::make_optional< std::vector<h2sl::Word> >();
//  h2sl::Symbol np_symbol = h2sl::Symbol( "object", {{"uid","ball0"}} );
  ...
  auto pp = std::make_shared<h2sl::LanguageVariable>("PP");
  pp->words = std::make_optional< std::vector<h2sl::Word> >();
  ...
  auto vp = std::make_shared<h2sl::LanguageVariable>("VP");
  vp->words = std::make_optional< std::vector<h2sl::Word> >();
  ...
  if( vm.count("input") ){
    std::cout << "Importing a Sentence from \"" << vm["input"].as<std::string>()
              << "\"" << std::endl;
    sentence.from_xml( vm["default"].as<std::string>().c_str(), symbolspace );
  } else{
