...
value( const unsigned int& cv,
  ...
  if( container_grounding != NULL ){
    if( container_grounding->container().size() > 1 ){
      //const World * h2sl_nsf_nri_mvli_world = dynamic_cast< const World* >( world );
      map< string, vector< Object* > >::const_iterator it;
      it = world->min_distance_sorted_objects().find( container_grounding->type() );
      //Can check lines 87 and 88?
      for( unsigned int i = 0; i < container_grounding->container().size(); i++ ){
        for( unsigned int j = 0; j < it->second[ i ]->size(); j++ ){
          if( container_grounding->container()[ i ] == it->second[ i ]->type() [ j ] ){
            if( j >= container_grounding->container().size() ){
              return _invert;
            }
          }
        }
