...
bool
Feature_Container_Dominant_Y_Shape::
container_expresses_dominant_y_structure( const std::vector < Grounding* >& container, 
                                          const double& model_deviation_tolerance, 
                                          const double& adjacent_distance_tolerance, 
  ...
  if ( dynamic_cast< const Object* >(container.front()) != NULL ) {
   ...
   // Check inter-element adjacent distance tolerance.
    ...
    for( unsigned int i = 0; i < (sorted_container.size() - 1); i++ ){
      if( abs( dynamic_cast< const Object* >(sorted_container[ (i + 1) ])->transform().position().y() - 
               dynamic_cast< const Object* >(sorted_container[ i ])->transform().position().y() ) > adjacent_distance_tolerance ){
        return false;
      }
    }
...
value( const unsigned int& cv,
  ...
  if (container != NULL) {
    if (container_expresses_dominant_y_structure(container->container(), 
