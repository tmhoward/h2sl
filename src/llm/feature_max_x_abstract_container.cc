            ...
            if( spatial_relation_child != NULL ){
                if( spatial_relation_child->type() == _relation_type ){
                    if( object_grounding != NULL ){
                        ...
                        for( unsigned int i = 0; i < h2sl_nsf_nri_mvli_world->max_x_sorted_objects()[ abstract_container_child->type() ].size(); i++ ){
                            if( *object_grounding == *h2sl_nsf_nri_mvli_world->max_x_sorted_objects()[ abstract_container_child->type() ][ i ] ){
                                if( i < abstract_container_child->num() ){
                                    return !_invert;
                                } else {
                                    return _invert;
                                }
