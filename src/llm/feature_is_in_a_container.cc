...
value( const unsigned int& cv,
  ...
  if( abstract_container_child != NULL ){
    if( object_grounding != NULL ){
      if( object_grounding->type() == abstract_container_child->type() ){
        ...
        switch( _container_type ){
          case Container::Type::TYPE_ROW:
            if( in_a_row( object_grounding, objects, object_grounding, abstract_container_child->num(), 0 ) ){
              return !_invert;
            } else{
              return _invert;
            }
          
          case Container::Type::TYPE_COLUMN:
            if( in_a_column( object_grounding, objects, object_grounding, abstract_container_child->num(), 0 ) ){
              return !_invert;
            } else{
              return _invert;
            }
          ...
          /*
          case Container::Type::TYPE_TOWER:
            //if( in_a_tower( object_grounding, objects, abstract_container_child->num(), 1 ) ){
            if( in_a_tower( ) ){
              return _invert;
            } else{
              return !_invert;
...
in_a_row( const Object* local_root_object, vector< Object* > all_objects, const Object* global_root_object, unsigned int container_size, unsigned int counter  ){
  ...
  for( unsigned int i = 0; i < all_objects.size(); i++ ){
    if( all_objects[ i ]->name() != local_root_object->name() ){
...
//      _container_type = strtol( container_type_string.c_str(), NULL, 10 );
