...
bool
Feature_Container_Dominant_Z_Shape::
container_expresses_dominant_z_structure( const std::vector < Object* >& container, 
                                          const double& model_deviation_tolerance, 
                                          const double& adjacent_distance_tolerance, 
 ...
 // Check inter-element adjacent distance tolerance.
  ...
  for( unsigned int i = 0; i < (sorted_container.size() - 1); i++ ){
    if( abs( sorted_container[ (i + 1) ]->transform().position().z() - sorted_container[ i ]->transform().position().z() ) > adjacent_distance_tolerance ){
      return false;
    }
  }
...
value( const string& cv,
  ...
  if (container != NULL) {
    ...
    if (container_expresses_dominant_z_structure( object_container, 
