/**
 * @file feature-spatial-relation-children-satisfied-object-type.cc
...
///
/// FeatureSpatialRelationChildrenSatisfiedObjectType default class constructor
///
FeatureSpatialRelationChildrenSatisfiedObjectType::FeatureSpatialRelationChildrenSatisfiedObjectType( const std::string& objectTypeArg ) :
...
///
/// FeatureSpatialRelationChildrenSatisfiedObjectType class constructor from an XMLElement
///
FeatureSpatialRelationChildrenSatisfiedObjectType::FeatureSpatialRelationChildrenSatisfiedObjectType( const tinyxml2::XMLElement* root ) : Feature(), object_type(){
  from_xml( root );
}
...
///
std::shared_ptr<Feature> FeatureSpatialRelationChildrenSatisfiedObjectType::dup( void )const{
  return std::make_shared<FeatureSpatialRelationChildrenSatisfiedObjectType>(*this);
}

///
/// Method to load from a tinyxml2 element
///
bool FeatureSpatialRelationChildrenSatisfiedObjectType::from_xml( const tinyxml2::XMLElement* root ){
  ...
  if( root == nullptr ){
    try {
      ...
      error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] XMLElement is nullptr in from_xml";
  ...
  // Check to see if the class name is feature-spatial-relation-children-satisfied-object-type
  const tinyxml2::XMLAttribute* class_attr = root->FindAttribute("class");
  if( class_attr == nullptr ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] could not find class attribute in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
  ...
  if( std::strcmp(class_attr->Value(), "feature-spatial-relation-children-satisfied-object-type") != 0 ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] Erroneous class name (\"" << class_attr->Value() << "\") in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
  ...
  if( object_type_attr == nullptr ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] Missing \"object_type\" attribute in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
...
FeatureValue FeatureSpatialRelationChildrenSatisfiedObjectType::evaluate( const std::shared_ptr< std::string >& cv,
                                        ...
                                        const std::shared_ptr<Symbol>& symbol )
{
  ...
  if( it_object_uid == symbol->properties.end() ){
    std::stringstream error_msg;
    error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] object symbol had no \"uid\" property";
    throw std::runtime_error( error_msg.str() );
  }
  ...
  std::string symbol_object_type = it_object_type->second;
  if( symbol_object_type != object_type ) return value;
  ...
  for( const auto& connection : lv->children() ){
    for( const auto& child_symbol : connection.child->symbols() ){
      ...
      if( it_axis == child_symbol->properties.end() ){
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] spatial_relation symbol had no \"axis\" property";
        throw std::runtime_error( error_msg.str() );
      } else if( it_axis->second.length() != 2 ){
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] spatial_relation symbol had invalid \"axis\" property value: "<< it_axis->second;
        throw std::runtime_error( error_msg.str() );
      }
      ...
      if( axis_direction == '+' || axis_direction == '-' ){
        ...
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] spatial_relation symbol had invalid \"axis\" direction: " << axis_direction;
        throw std::runtime_error( error_msg.str() );
      }
      ...
      if( !( landmark_uid || viewpoint_uid ) ){
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationChildrenSatisfiedObjectType Class Error] spatial_relation symbol had neither a landmark nor viewpoint";
        throw std::runtime_error( error_msg.str() );
      }
...
///
std::string FeatureSpatialRelationChildrenSatisfiedObjectType::print_string( const bool& printValue )const{
  std::stringstream out;
  if( printValue ){
    out << "feature-spatial-relation-children-satisfied-object-type(type=\"" << type << "\",value=\"" << value << "\",object_type=\"" << object_type << "\")";
  } else {
    out << "feature-spatial-relation-children-satisfied-object-type(type=\"" << type << "\",object_type=\"" << object_type << "\")";
  }
  return out.str();
}
...
///
std::string FeatureSpatialRelationChildrenSatisfiedObjectType::key( void ) const{
  std::stringstream out;
  out << "feature-spatial-relation-children-satisfied-object-type(type=\"" << type << "\",object_type=\"" << object_type << "\")";;
  return out.str();
}
...
///
bool FeatureSpatialRelationChildrenSatisfiedObjectType::from_json_string( const std::string& valueArg ){
  Json::Reader reader;
  Json::Value root;
...
///
bool FeatureSpatialRelationChildrenSatisfiedObjectType::from_json( const Json::Value& root ){
  return true;
}
...
///
void FeatureSpatialRelationChildrenSatisfiedObjectType::to_xml( tinyxml2::XMLDocument& document, tinyxml2::XMLElement* root )const{
  tinyxml2::XMLElement* feature_elem = document.NewElement( "feature" );
  feature_elem->SetAttribute( "class", "feature-spatial-relation-children-satisfied-object-type" );
...
///
std::string FeatureSpatialRelationChildrenSatisfiedObjectType::to_json( void )const{
  Json::Value root;
  to_json( root );
...
///
void FeatureSpatialRelationChildrenSatisfiedObjectType::to_json( Json::Value& root )const{
  return;
}

///
/// FeatureSpatialRelationChildrenSatisfiedObjectType class ostream operator
///
std::ostream& operator<<(std::ostream& out, const FeatureSpatialRelationChildrenSatisfiedObjectType& other) {
  other.print( out );
  return out;
}
