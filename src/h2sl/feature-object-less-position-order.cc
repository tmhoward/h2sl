/**
 * @file feature-object-less-position-order.cc
...
///
/// FeatureObjectLessPositionOrder argument constructor
///
FeatureObjectLessPositionOrder::
FeatureObjectLessPositionOrder( const World::SortKey& sortKeyArg, const int offsetArg ) :
...
///
FeatureObjectLessPositionOrder::
FeatureObjectLessPositionOrder( const tinyxml2::XMLElement* root ) :
  Feature(),
  sort_key(),
  offset()
{
...
///
std::shared_ptr<Feature> FeatureObjectLessPositionOrder::dup( void )const{
  return std::make_shared<FeatureObjectLessPositionOrder>(*this);
}

///
/// Method to load from a tinyxml2 element
///
bool FeatureObjectLessPositionOrder::from_xml( const tinyxml2::XMLElement* root ){
  ...
  // Check that the element is a feature-object-less-position-order element
  if( root == nullptr ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureObjectLessPositionOrder Class Error] XMLElement is nullptr in from_xml";
  ...
  if( class_attr == nullptr ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureObjectLessPositionOrder Class Error] could not find class attribute in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
  ...
  if( std::strcmp(class_attr->Value(), "feature-object-less-position-order") != 0 ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureObjectLessPositionOrder Class Error] Erroneous class name (\"" << root->Name() << "\") in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
  ...
  if( sort_key_attr == nullptr ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureObjectLessPositionOrder Class Error] Missing \"sort_key\" attribute in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
  ...
  if( offset_attr == nullptr ){
    std::stringstream error_msg;
    error_msg << "[FeatureObjectLessPositionOrder Class Error] Missing \"offset\" attribute in from_xml";
    throw std::runtime_error( error_msg.str() );
  }
...
///
FeatureValue
FeatureObjectLessPositionOrder::evaluate( const std::shared_ptr< std::string >& cv,
                                      ...
                                      const std::shared_ptr<Symbol>& symbol )
{
  ...
  // Check if the object matches any element in the sorted objects up to the feature's specified offset
  for( auto cit_sorted_object = it_map_sorted_objects_object_type->second.cbegin();
        cit_sorted_object != it_map_sorted_objects_object_type->second.cbegin() + offset + 1;
        ++cit_sorted_object )
  {
    if( object_symbol.uid == (*cit_sorted_object)->uid )
                                 ...
                                 const std::shared_ptr<World>& world)const
{
  ...
  std::stringstream error_msg;
  error_msg << "[FeatureObjectLessPositionOrder Class Error] ";
...
///
std::string FeatureObjectLessPositionOrder::print_string( const bool& printValue )const{
  std::stringstream out;
  if( printValue ){
    out << "feature-object-less-position-order(type=\"" << type << "\",value=\"" << value << "\",sort_key=\"" << World::sortkey_to_string( sort_key ) << "\",offset=\"" << offset << "\")";
  } else {
    out << "feature-object-less-position-order(type=\"" << type << "\",sort_key=\"" << World::sortkey_to_string( sort_key ) << "\",offset=\"" << offset << "\")";
  }
  return out.str();
}
...
///
std::string FeatureObjectLessPositionOrder::key( void ) const{
  std::stringstream out;
  out << "feature-object-less-position-order(type=\"" << type << "\",sort_key=\"" << World::sortkey_to_string( sort_key ) << "\",offset=\"" << offset << "\")";
  return out.str();
}
...
///
bool FeatureObjectLessPositionOrder::from_json_string( const std::string& valueArg ){
  Json::Reader reader;
  Json::Value root;
...
///
bool FeatureObjectLessPositionOrder::from_json( const Json::Value& root ){
  return true;
}
...
///
void FeatureObjectLessPositionOrder::to_xml( tinyxml2::XMLDocument& document, tinyxml2::XMLElement* root )const{
  tinyxml2::XMLElement* feature_object_merge_object_type_region_elem = document.NewElement( "feature" );
  feature_object_merge_object_type_region_elem->SetAttribute( "class", "feature-object-less-position-order" );
...
///
std::string FeatureObjectLessPositionOrder::to_json( void )const{
  Json::Value root;
  to_json( root );
...
///
void FeatureObjectLessPositionOrder::to_json( Json::Value& root )const{
  return;
}
