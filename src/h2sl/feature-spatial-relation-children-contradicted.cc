/**
 * @file feature-spatial-relation-children-contradicted.cc
...
///
FeatureSpatialRelationChildrenContradicted::FeatureSpatialRelationChildrenContradicted( void ) :
...
///
FeatureSpatialRelationChildrenContradicted::FeatureSpatialRelationChildrenContradicted( const tinyxml2::XMLElement* root ) : Feature(){
  from_xml( root );
}
...
///
std::shared_ptr<Feature> FeatureSpatialRelationChildrenContradicted::dup( void )const{
  return std::make_shared<FeatureSpatialRelationChildrenContradicted>(*this);
}

///
/// Method to load from a tinyxml2 element
///
bool FeatureSpatialRelationChildrenContradicted::from_xml( const tinyxml2::XMLElement* root ){
  ...
  if( root == nullptr ){
    try {
      ...
      error_msg << "[FeatureSpatialRelationChildrenContradicted Class Error] XMLElement is nullptr in from_xml";
  ...
  // Check to see if the class name is feature-spatial-relation-children-contradicted
  const tinyxml2::XMLAttribute* class_attr = root->FindAttribute("class");
  if( class_attr == nullptr ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureSpatialRelationChildrenContradicted Class Error] could not find class attribute in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
  ...
  if( std::strcmp(class_attr->Value(), "feature-spatial-relation-children-contradicted") != 0 ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureSpatialRelationChildrenContradicted Class Error] Erroneous class name (\"" << class_attr->Value() << "\") in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
...
FeatureValue FeatureSpatialRelationChildrenContradicted::evaluate( const std::shared_ptr< std::string >& cv,
                                        ...
                                        const std::shared_ptr<Symbol>& symbol )
{
  ...
  if( it_object_uid == symbol->properties.end() ){
    std::stringstream error_msg;
    error_msg << "[FeatureSpatialRelationChildrenContradicted Class Error] object symbol had no \"uid\" property";
    throw std::runtime_error( error_msg.str() );
  }
  ...
  for( const auto& connection : lv->children ){
    for( const auto& child_symbol : connection.child->symbols ){
      ...
      if( it_axis == child_symbol->properties.end() ){
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationChildrenContradicted Class Error] spatial_relation symbol had no \"axis\" property";
        throw std::runtime_error( error_msg.str() );
      } else if( it_axis->second.length() != 2 ){
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationChildrenContradicted Class Error] spatial_relation symbol had invalid \"axis\" property value: "<< it_axis->second;
        throw std::runtime_error( error_msg.str() );
      }
      ...
      if( !( landmark_uid || viewpoint_uid ) ){
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationChildrenContradicted Class Error] spatial_relation symbol had neither a landmark nor viewpoint";
        throw std::runtime_error( error_msg.str() );
      }
...
///
std::string FeatureSpatialRelationChildrenContradicted::print_string( const bool& printValue )const{
  std::stringstream out;
  if( printValue ){
    out << "feature-spatial-relation-children-contradicted(type=\"" << type << "\",value=\"" << value << "\")";
  } else {
    out << "feature-spatial-relation-children-contradicted(type=\"" << type << "\")";
  }
  return out.str();
}
...
///
std::string FeatureSpatialRelationChildrenContradicted::key( void ) const{
  std::stringstream out;
  out << "feature-spatial-relation-children-contradicted(type=\"" << type << "\")";
  return out.str();
}
...
///
bool FeatureSpatialRelationChildrenContradicted::from_json_string( const std::string& valueArg ){
  Json::Reader reader;
  Json::Value root;
...
///
bool FeatureSpatialRelationChildrenContradicted::from_json( const Json::Value& root ){
  return true;
}
...
///
void FeatureSpatialRelationChildrenContradicted::to_xml( tinyxml2::XMLDocument& document, tinyxml2::XMLElement* root )const{
  tinyxml2::XMLElement* feature_elem = document.NewElement( "feature" );
  feature_elem->SetAttribute( "class", "feature-spatial-relation-children-contradicted" );
...
///
std::string FeatureSpatialRelationChildrenContradicted::to_json( void )const{
  Json::Value root;
  to_json( root );
...
///
void FeatureSpatialRelationChildrenContradicted::to_json( Json::Value& root )const{
  return;
}
