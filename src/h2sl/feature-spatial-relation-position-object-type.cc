/**
 * @file feature-spatial-relation-position-object-type.cc
...
///
std::shared_ptr<Feature> FeatureSpatialRelationPositionObjectType::dup( void )const{
  return std::make_shared<FeatureSpatialRelationPositionObjectType>(*this);
}

///
/// Method to load from a tinyxml2 element
///
bool FeatureSpatialRelationPositionObjectType::from_xml( const tinyxml2::XMLElement* root ){
  ...
  if( root == nullptr ){
    try {
      ...
      error_msg << "[FeatureSpatialRelationPositionObjectType Class Error] XMLElement is nullptr in from_xml";
  ...
  // check to see if the class name is feature-spatial-relation-position-object-type
  const tinyxml2::XMLAttribute* class_attr = root->FindAttribute("class");
  if( class_attr == nullptr ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureSpatialRelationPositionObjectType Class Error] could not find class attribute in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
  ...
  if( std::strcmp(class_attr->Value(), "feature-spatial-relation-position-object-type") != 0 ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureSpatialRelationPositionObjectType Class Error] Erroneous class name (\"" << class_attr->Value() << "\") in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
  ...
  if( position_attr == nullptr ){
    try {
      std::stringstream error_msg;
      error_msg << "[FeatureSpatialRelationPositionObjectType Class Error] Missing \"position_index\" attribute in from_xml";
      throw std::runtime_error( error_msg.str() );
    } catch ( const char* msg ){
      std::cerr << msg << std::endl;
    }
...
FeatureValue FeatureSpatialRelationPositionObjectType::evaluate( const std::shared_ptr< std::string >& cv,
                                        ...
                                        const std::shared_ptr<Symbol>& symbol )
{
  ...
  if( it_object_uid == symbol->properties.end() ){
    std::stringstream error_msg;
    error_msg << "[FeatureSpatialRelationPositionObjectType Class Error] object symbol had no \"uid\" property";
    throw std::runtime_error( error_msg.str() );
  }
  ...
  for( const auto& connection : lv->children() ){
    for( const auto& child_symbol : connection.child->symbols() ){
      ...
      if( it_axis == child_symbol->properties.end() ){
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationPositionObjectType Class Error] spatial_relation symbol had no \"axis\" property";
        throw std::runtime_error( error_msg.str() );
      } else if( it_axis->second.length() != 2 ){
        std::stringstream error_msg;
        error_msg << "[FeatureSpatialRelationPositionObjectType Class Error] spatial_relation symbol had invalid \"axis\" property value: "<< it_axis->second;
        throw std::runtime_error( error_msg.str() );
      }
...
///
bool FeatureSpatialRelationPositionObjectType::from_json_string( const std::string& valueArg ){
  Json::Reader reader;
  Json::Value root;
...
///
bool FeatureSpatialRelationPositionObjectType::from_json( const Json::Value& root ){
  return true;
}
...
///
void FeatureSpatialRelationPositionObjectType::to_xml( tinyxml2::XMLDocument& document, tinyxml2::XMLElement* root )const{
...
///
std::string FeatureSpatialRelationPositionObjectType::to_json( void )const{
  Json::Value root;
  to_json( root );
...
///
void FeatureSpatialRelationPositionObjectType::to_json( Json::Value& root )const{
  return;
}
